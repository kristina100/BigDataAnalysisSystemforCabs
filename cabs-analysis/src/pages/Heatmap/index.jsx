...
export default class HeatMap extends Component {
    componentDidMount() {
        ...
        map.plugin(["AMap.HeatMap", "AMap.DistrictSearch"], function () {      //加载热力图插件
        ...
        window.AMapUI.load(['ui/misc/PointSimplifier'], function (PointSimplifier) {
            ...
            try {
                ...
                const drawBounds = (districtName) => {
                    if (lastDistrictName === districtName) {
                        map.remove(polygons)//清除上次结果
                        let position = new window.AMap.LngLat(113.5683, 23.12897);  // 标准写法
                        ...
                        for (let i = 0; i < 11; i++) {
                            ...
                            typeBtn[i].onclick = () => {
                                if (isLoading) {
                                    ...
                                    drawBounds(typeBtn[i].innerText);
                                    // flowPointShow(i);
                                    flowGraphShow(i);
                                }
                            }
